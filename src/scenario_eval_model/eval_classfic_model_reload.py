...
from data_load import EdgeBatchDataLoader,EdgeBatchDataLoader_new
...
def evaluate_model(model, system_name_list, model_path):
    ...
    for system_name in system_name_list:
        result_list = []
        data_dir_path = f'/workspace3/sceanrio_data/scenario_data_2024030318_nofp/{system_name}'
        ...
        test_loader = EdgeBatchDataLoader_new(test_data, batch_size=1, shuffle=True)
        test_loss = 0
        correct = 0
        total = 0
        TP = FP = TN = FN = 0
        ...
        with torch.no_grad():
            for batch in test_loader:
                ...
                for i in range(len(predicted)):
                    ...
                    file_path = batch.json_path[i] if 'json_path' in batch else None
                    dir_path = os.path.dirname(file_path)
                    f_dir_path =os.path.dirname(dir_path)
                    file_base_name = os.path.basename(file_path)
                    

                    sem_score = batch.predict_score[i].cpu().squeeze().numpy() if 'predict_score' in batch else None
                    sem_label = batch.predict_label[i].cpu().squeeze().numpy() if 'predict_label' in batch else None

                    predicted_score = out[i].cpu().squeeze().numpy()

                    predicted_label = predicted[i].cpu().squeeze().numpy()

                    true_label = target[i].cpu().squeeze().numpy()

                    error_type = batch.error_type[i] if 'error_type' in batch else None
                    result_list.append({'system':f'{system_name}',
        ...
        #按照 system,result,predicted score,dir path,file path的顺序排序
        result_df = result_df.sort_values(by=['system','result','dir path','file path'],ascending=[True,True,True,True])
        result_df.to_csv(f'{data_dir_path}/result.csv', index=False)
...
system_name_list = ['leaderboard-LAV']
#system_name_list = ['behavior','leaderboard-NEAT','basic','leaderboard-LAV', 'leaderboard-Transfuser','leaderboard-LAV_V2','leaderboard-Transfuser_V2']
model_path = f'src/scenario_eval_model/model_dir/improve_v{MODEL_VERSION}/gnn_binary_classification_model_{MODEL_TYPE}.pt'
#model_path ='/workspace3/SEM_train/all_data/run_20240306-134731//gnn_binary_classification_model_1.pt'
