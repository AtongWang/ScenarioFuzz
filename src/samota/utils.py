...
def get_top_five_according_jaccard_distance(value, temp_X):
    ...
    sorted_data = sorted(all_feature_vectors_with_distances, key=lambda x: x[12])
...
def generate_one_cluster(file_name, clean_data, percent, index):
    ...
    X = dataset.iloc[:, 0:11].values.tolist()
...
def generate_clusters_one_for_top_n(file_name, clean_data, percent, index):
    ...
    for value in X:
        ...
        top_five = [x[0:12] for x in top_five]
...
def create_database(all_data,clean_data):
    database = []
    pre_process_data(all_data, 100, 11, clean_data)
...
def preprocess_data(database2,percent,index):
    ...
    file_name = os.path.join(STORE_DIR, 'clean_data.csv')
...
def generate_clusters_using_database(database, percent, index):
    ...
    for label in np.unique(clusterer.labels_):
        array = values_with_label(X_scaled, label, clusterer.labels_, Y)
        all_data_clusters.append(array)
    if len(np.unique(clusterer.labels_)) == 1 and np.unique(clusterer.labels_)[0] == -1:
        return all_data_clusters, -1

    return all_data_clusters, len(np.unique(clusterer.labels_)) - 1
...
def values_with_label(X, label, labels, values):
    ...
    for val in range(len(labels)):
        if labels[val] == label:
            ...
            #print(arr)
            # 将 arr 转换为二维数组
            arr = arr.reshape(1, -1)  # 重塑 arr 为二维数组
            arr = scaler.inverse_transform(arr)
            arr = arr.flatten()  # 将 arr 再次转换为一维数组以便后续处理
...
def find_bounds(cluster):
    ...
    lb = [2, 0, 1, 1, 1, 1, 1, 1, 2, 6, 1]

    for feature_vector in cluster:
        fv = feature_vector[0:11]
