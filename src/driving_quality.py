...
def get_ay_gain(SWA_heavy_list, Ay_heavy):
    ...
    if len(SWA_heavy_list) - len(Ay_heavy):
        Ay_ext = np.append(Ay_heavy, np.zeros(len(SWA_heavy_list) - len(Ay_heavy))) # pad with zeros to match length
    else:
        Ay_ext = Ay_heavy
    Ay_gain = np.absolute(
            np.divide(
    ...
    return Ay_gain # trim to match length
