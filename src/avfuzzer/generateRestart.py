...
from src.avfuzzer.chromosome import Chromosome
import src.avfuzzer.tools as tools
...
def generateRestart(ck_path, scenarioNum, bounds, chromosome_object):
	...
	for i in range(scenarioNum):
		speed_bounds = bounds[0]
		action_bounds = bounds[1]
		chromosome = chromosome_object(speed_bounds,action_bounds,npcSize, len(prevPopPool[0][0].scenario[0]))
