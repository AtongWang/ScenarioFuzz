...
def draw_waypoints(waypoints, color):
    ...
    for wp in waypoints:
        world.debug.draw_string(wp.transform.location, 'O', draw_shadow=False,
                                color=carla.Color(r=color[0], g=color[1], b=color[2]), life_time=10.0,
                                persistent_lines=True)
...
def get_route_direction(component,debug=False):
    def angle_diff(a, b):
    ...
    start_wp = path[0]
    end_wp = path[-1]
    start_lane_id = int(start_wp.lane_id)
    end_lane_id = int(end_wp.lane_id)
    # start_lane_type = type2int(start_wp.lane_change)
    # end_lane_type = type2int(end_wp.lane_change)
    # left_lane_list = [-1,0,2,3]
    # right_lane_list = [1,0,2,4]
    # 计算路径的方向向量
    start_loc = start_wp.transform.location
    end_loc = end_wp.transform.location
    start_yaw =start_wp.transform.rotation.yaw
    end_yaw = end_wp.transform.rotation.yaw
    # dx = end_loc.x - start_loc.x
    # dy = end_loc.y - start_loc.y
    if debug:
        spectator = world.get_spectator()
        world_snapshot = world.wait_for_tick() 
        sp_tf =carla.Transform(location=carla.Location(z=50,x=start_loc.x,y=start_loc.y),rotation=carla.Rotation(pitch=-90)) 
        spectator.set_transform(sp_tf)
        draw_waypoints([start_wp,end_wp], color=[255, 0, 0]) 
        world.debug.draw_string(start_wp.transform.location,'test_find_way', draw_shadow=False,
                            color=carla.Color(*(255,0,0)), life_time=10.0,
                            persistent_lines=True)
    # 计算路径的方向角度
    angle = angle_diff(end_yaw, start_yaw)
    # if angle < -180:
    #     angle += 360
    # elif angle > 180:
    #     angle -= 360
    distance = start_loc.distance(end_loc)
    revesed =get_way_reverse(start_wp,end_wp)
    
    # 根据路径的方向角度判断路径的方向
    if -5 <= angle <= 5:
        return "Straight",distance,revesed
    elif -100 <= angle < -5.0 :
        return "Left",distance,revesed
    elif 5 < angle <= 100.0:
        return "Right",distance,revesed
    else:
        return "Unknown",distance,revesed

def get_way_reverse(start_waypoint, end_waypoint, max_iterations=100):
    ...
    for _ in range(max_iterations):
        ...
        if not next_waypoints:
            # 如果没有下一个waypoint，说明无法到达
            return False

        # 选择距离终点最近的下一个waypoint
        current_waypoint = min(next_waypoints, key=lambda wp: wp.transform.location.distance(end_waypoint.transform.location))

        # 如果到达终点附近，返回True
        if current_waypoint.transform.location.distance(end_waypoint.transform.location) < 1.0:
            return True
...
def main(town):
    ...
    def can_spawn_actor(world, spawn_transform, actor_blueprint):
        can_spawn = False
        actor = None
        try:
            actor = world.try_spawn_actor(actor_blueprint, spawn_transform)
            if actor:
                can_spawn = True
        except carla.TrafficError as e:
            pass
        finally:
            if actor:
                actor.destroy()
        ...
        return can_spawn
    actor_blueprint = world.get_blueprint_library().filter("vehicle.*")[0]
    ...
    for waypoint_pair in topology:
        ...
        node1_transform = waypoint1.transform
        node2_transform = waypoint2.transform
        
        if can_spawn_actor(world, node1_transform, actor_blueprint):
            node1 = (node1_transform.location.x, node1_transform.location.y, node1_transform.location.z)
            G.add_node(node1)
            
        if can_spawn_actor(world, node2_transform, actor_blueprint):
            node2 = (node2_transform.location.x, node2_transform.location.y, node2_transform.location.z)
            G.add_node(node2)
    ...
    for waypoint_pair in topology:
        ...
        if node1 in G.nodes and node2 in G.nodes:
            distance = carla.Location(node1[0],node1[1],node1[2]).distance(carla.Location(node2[0],node2[1],node2[2]))
            G.add_edge(node1, node2, weight=distance)

        if len(G.nodes) ==0:
            norm_generate(topology,G)
    ...
    new_scenario_dict_tf = {}
    new_key = 0

    for t in range(4):
        for i, v in scenario_dict_tf.items():
            new_item = {'type': None, 'wp': [],'wp_mark':[], 'wp_lanetype':[],'tf_loc': v['tf_loc'],'center_loc':None,'plan_way':[]}
            for tf_loc in v['tf_loc']:
                ...
                if wp_i > 50:
                    ...
                    new_item['wp'].append(node_list[wp_i])
                    new_item['wp_mark'].append(n_mark_name)
                    new_item['wp_lanetype'].append(n_wp_lane_type)
                    node_h[wp_i] = 1
            if len(new_item['wp']) != 0:
                new_scenario_dict_tf[new_key] = new_item
                new_key += 1

    scenario_dict_tf = new_scenario_dict_tf
    ...
    for index, n_o_class_nodes in enumerate(node_others_classes):
        ...
        for index_2, com in enumerate(compoents_list):
            if len(com) == 1:
                ...
                direction,lenth,reveres = get_route_direction(com)
                drive_way_s.append((direction,lenth,com,reveres))
    ...
    if all([len_i == 1 for len_i in [len(i) for i in compoents_last ]]):
        ...
        for index_2, com in enumerate(compoents_last):
            if len(com) >1:
                direction,lenth,reveres = get_route_direction(com)
                drive_way_last.append((direction,lenth,com,reveres))
    ...
    for sc in scenario_dict_tf.values():
        if len(sc['wp'])<=30:
            ...
            for com_l in compoents_list:
                pl_d,lenth,reveres =get_route_direction(com_l)
                if (pl_d,lenth,com_l) not in sc['plan_way']:
                    sc['plan_way'].append((pl_d,lenth,com_l,reveres))
...
# -----------针对缺少直线道路的情况，由于前代码所有的思路均是以路口为主，故从全图搜索plan，为路径较长，且一直直行的wp，存入sceanriodict
    ...
    for ind in range(len(sorted_idx)):
        ...
        pl_d,lenth,reveres =get_route_direction(pl_w)
        if pl_d =='Straight' and lenth>30:
            ...
            for n_o in n_o_wp:
                ...
                n_mark_name = [i.name for i in n_mark] if len(n_mark)   !=0 else []
                v['wp_mark'].append(n_mark_name)
            v['plan_way'].append((pl_d,lenth,pl_w,reveres))
            v['center_loc'] = center_cal([loc for loc in v['wp']])
...
if __name__ == "__main__":
    for town in  ['Town03','Town02','Town01','Town04','Town05']:
        try:
            main(town) 
        except:
            print(f'{town} is not success!')