...
class BaseReader(object):
    def __init__(self, vehicle,world,clock,reading_frequency=1.0):
        self._clock =  clock
        self._world = world
    ...
    def run(self):
        first_time = True
        latest_time = self._clock.get_time()
        while self._run_ps:
            if self._callback is not None:
                ...
                if current_time - latest_time > (1 / self._reading_frequency) \
                        or (first_time and self._clock.get_frame()!= 0):
                    self._callback(GenericMeasurement(self.__call__(), self._clock.get_frame()))
                    latest_time = self._clock.get_time()
                    first_time = False
...
class OpenDriveMapReader(BaseReader):
    def __call__(self):
        return {'opendrive': self._world.get_map().to_opendrive()}
...
class SensorInterface(object):
    ...
    def get_data(self):
        try: 
            ...
            print(f"num:{len(data_dict.keys())},type:{data_dict.keys()}")
