...
from LAV_V2.lav.models.lidar import LiDARModel
from LAV_V2.lav.models.uniplanner import UniPlanner
from LAV_V2.lav.models.bev_planner_v2 import BEVPlanner
from LAV_V2.lav.models.rgb_v2 import RGBSegmentationModel, RGBBrakePredictionModel
from LAV_V2.team_code.pid import PIDController
from LAV_V2.team_code.ekf import EKF, move_lidar_points
from LAV_V2.team_code.point_painting import CoordConverter, point_painting
from LAV_V2.team_code.planner import RoutePlanner
from LAV_V2.team_code.waypointer import Waypointer
from LAV_V2.team_code.model_inference import InferModel
...
class LAVAgent(AutonomousAgent):
    ...
    def setup(self, path_to_conf_file,save_path=None,analysis=False,record_action=False,load_action_file=None):
        ...
        (self.save_path / 'log').mkdir(parents=True, exist_ok=False)
    ...
    def flush_data(self):
        ...
        # We assume that the images are 480x640 and the output video is 30 fps
        height, width, layers = self.vizs[0].shape
        size = (width,height)
        out = cv2.VideoWriter(str(self.save_path / 'log' /'project.mp4' ),cv2.VideoWriter_fourcc(*'mp4v'), 20, size)

        for i in range(len(self.vizs)):
            # Convert RGB to BGR
            bgr_frame = cv2.cvtColor(self.vizs[i], cv2.COLOR_RGB2BGR)
        ...
        if self.log_wandb:
            ...
            wandb.save(str(self.save_path / 'log' / 'wandb.h5'))
        self.vizs.clear()
