...
class TransFuserAgent(AutonomousAgent):
	pred_wp =[None]
	def setup(self, path_to_conf_file, save_path=None,analysis=False,record_action=False,load_action_file=None):
		...
		self.net = TransFuser(self.config, self.device)
	...
	def save(self, tick_data):
		...
		#print(np.shape(self.lidar_processed[0].cpu().numpy()))
	...
	def save_analysis(self, tick_data):
		...
		np.save(self.save_path / 'lidar' / ('%04d.npy' % frame),self.lidar_processed[0].cpu().numpy()[0,...],allow_pickle=True)
		self.meta_data = {
		'x':tick_data['gps'][0],
		'y':tick_data['gps'][1],
