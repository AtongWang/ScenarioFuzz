...
from Transfuser_V2.model import LidarCenterNet
from Transfuser_V2.config import GlobalConfig
from Transfuser_V2.data import lidar_to_histogram_features, draw_target_point, lidar_bev_cam_correspondences
...
class HybridAgent(AutonomousAgent):
    pred_wp =[None]
    def setup(self, path_to_conf_file, save_path=None,analysis=False,record_action=False,load_action_file=None,route_index=None):
    ...
    def sensors(self):
        ...
        if(self.save_path != None): #Debug camera for visualizations
            sensors.append({
                            'type': 'sensor.camera.rgb',
                            'x': -4.5, 'y': 0.0, 'z':2.3,
            ...
            for i in range(self.model_count):
                ...
                if (self.backbone == 'transFuser'):
                    pred_wp, _ = self.nets[i].forward_ego(image, lidar_bev, target_point, target_point_image, velocity,
                                                          num_points=num_points, save_path=self.save_path, stuck_detector=self.stuck_detector,
    ...
    def make_video(self,fps=20):
        files = sorted(glob.glob(os.path.join(self.save_path,'*.png')), key=lambda x: int(os.path.basename(x).split('.')[0]))
           
        if len(files) == 0:
            print(f"No Image files found in {self.save_path}")
            return

        img = cv2.imread(files[0])
        height, width, _ = img.shape
        output_path = os.path.join(self.save_path, "project.mp4")
