...
from NEAT.neat.architectures import AttentionField
from NEAT.neat.config import GlobalConfig
from NEAT.neat.utils import flow_to_color
from NEAT.team_code.planner import RoutePlanner
sys.path.append('..')
from autonomous_agent import AutonomousAgent,Track
from utils import interpolate_trajectory,downsample_route
...
class MultiTaskAgent(AutonomousAgent):

	def setup(self, path_to_conf_file,save_path=None,analysis=False,record_action=False,load_action_file=None):
		self.name = get_entry_point()
		self.pred_wp = [None]
		...
		self.load_action_file = load_action_file
		...
		self.device = torch.device(self._clock.device)
		...
		self.net.eval()
		string = self._clock.file_name+'-{}'.format( time.time())
		self.save_path = None
		if save_path is not None:
			self.save_path = pathlib.Path(save_path) / string
		else:
			print('[error info] {} agent type don\'t have save_path!'.format(get_entry_point()))
			save_path = '/tmp/fuzzerdata'
			self.save_path = pathlib.Path(save_path) / string
		print('{} agent save_path is set at {}'.format(get_entry_point(),self.save_path))
		...
		if self.analysis:
			...
			(self.save_path / 'rgb_left').mkdir(parents=True, exist_ok=False)
		...
		if self.load_action_file is not None:
			self.load_action = True
			self.action_list = np.load(self.load_action_file,allow_pickle=True).item()
		else:
	...
	def set_destination(self, start_location, end_location, world, clean=False):
		"""
		...
		"""
		trajectory = [start_location,end_location]
	...
	def run_step(self, input_data, timestamp):
		...
		target_point = torch.stack(tick_data['target_point']).to(self.device, dtype=torch.float32)
		...
		self.pred_wp = pred_waypoint_mean
		...
		if not self.load_action:
			...
			control = carla.VehicleControl()
			control.steer = float(self.action_list[self.step][0])
			control.throttle = float(self.action_list[self.step][1])
			control.brake = float(self.action_list[self.step][2])
		if self.record_action :
			self.action_list[self.step]=(steer,throttle,brake)

		if  self.analysis:
			self.save_analysis(tick_data)
		else:
	...
	def save(self, tick_data):
		...
		for i in range(self.config.tot_len):
			...
			pred_waypoint = self.pred_wp[0,i].data.cpu().numpy()
			...
			for row in range(self.args['out_res']):
				...
				grid_points = grid_points.reshape(1,-1,3).to(self.device, dtype=torch.float32)
	...
	def save_analysis(self, tick_data):
		frame = self.step
		#rgb_front
		...
		'x_command': float(tick_data['target_point'][0]),
        'y_command': float(tick_data['target_point'][1]),
		'steer':self.pid_metadata['steer'],
		'throttle':self.pid_metadata['throttle'],
		'brake':self.pid_metadata['brake'],
		'command':tick_data['next_command'] ,
		'speed': tick_data['speed']
		}

		outfile = open(self.save_path / 'meta' / ('%04d.json' % frame), 'w')
		json.dump(self.meta_data,outfile, indent=4)
		outfile.close()
	def destroy(self):
		if self.record_action:
			np.save(self.save_path/ ('action.npy'),self.action_list,allow_pickle=True)
		del self.net
